...
for batch in train_loader: sum_of_squared_error += (
    (batch[0] - mean).pow(2)).sum()
...
class RunBuilder():
    ...
    def get_runs(params):
        ...
        runs = []
        for v in product(
                *params.values()):  # create a Cartesian product and return an ordered pairs that define our run
            runs.append(Run(
                *v))  # use * to tell the constructor to accept the tuple values as argument rather than the tuple itself
        return runs
